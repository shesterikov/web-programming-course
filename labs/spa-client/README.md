...
- Закрепить навыки контейнеризации: собрать статику клиента в отдельный образ и развернуть ее в кластере Kubernetes.
...
- Наличие кластера Kubernetes из работы [Масштабирование веб-приложений на примере Kubernetes](../kubernetes).
...
### Ход работы
В рамках данной работы необходимо разработать клиентское приложение, потребляющее API, созданное в предшествующих работах курса, и развернуть его в кластере, подготовленном в работе [Масштабирование веб-приложений на примере Kubernetes](../kubernetes).
...
Добавьте сервис клиента в существующий `docker-compose.yml`. Все приложение целиком должно подниматься командой `docker-compose up --build`.

#### 10. Развертывание в Kubernetes
Разверните клиент в кластере, подготовленном в работе [Масштабирование веб-приложений на примере Kubernetes](../kubernetes):
1.  Соберите образ клиента и создайте манифесты `Deployment` и `Service` в директории `k8s/07-web/` по образцу манифестов сервиса `api`.
2.  Настройте веб-сервер клиента на проксирование запросов с префикса `/api` на сервис `api` внутри кластера (адрес вида `http://api:4200`). При такой схеме браузер обращается к клиенту и к API по одному источнику, и механизм CORS в кластере не задействуется вовсе.
3.  Соберите клиент с относительным базовым адресом API (`VITE_API_URL=/api`), чтобы запросы уходили на тот же источник, с которого загружена страница.
4.  Пробросьте порт сервиса клиента и проверьте работу приложения:
    ```bash
    kubectl port-forward svc/web 8080:80 -n wp-labs
    ```
5.  Выполните горизонтальное масштабирование клиента и убедитесь, что приложение продолжает работать:
    ```bash
    kubectl scale deployment/web --replicas=3 -n wp-labs
    ```

Обратите внимание, что при работе через один источник атрибут `SameSite` перестает влиять на передачу cookies, а заголовки CORS в ответах отсутствуют. Сравните это поведение с локальным запуском из раздела 9 и будьте готовы объяснить разницу.
...
2.  Документация: Файл `README.md` клиента содержит:
    ...
    -   Инструкцию по развертыванию клиента в кластере Kubernetes.
3.  Инфраструктура:
    -   Клиент и API поднимаются одной командой и работают на разных портах.
    -   Клиент собран в образ с веб-сервером через многоэтапную сборку.
    -   Прямое открытие приватного маршрута и перезагрузка страницы не приводят к ошибке `404`.
    -   В кластере развернуты `Deployment` и `Service` клиента, приложение доступно через проброс порта.
4.  Аутентификация:
...
8.  Демонстрация: Студент проводит сценарий из раздела 11 с открытой вкладкой Network и объясняет наблюдаемые запросы.
...
12. Почему переменные окружения Vite подставляются на этапе сборки? Какие последствия это имеет для Docker-образа и для развертывания в кластере?
13. Зачем веб-серверу возвращать `index.html` для неизвестных маршрутов при раздаче одностраничного приложения?
14. Почему при проксировании запросов к API через веб-сервер клиента механизм CORS перестает задействоваться?
15. В чем преимущество генерации клиентского кода из спецификации OpenAPI перед рукописным описанием типов?
...
- Развертывание: [Nginx: Try Files](https://nginx.org/ru/docs/http/ngx_http_core_module.html#try_files), [Kubernetes Deployments](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/), [Kubernetes Services](https://kubernetes.io/docs/concepts/services-networking/service/).
