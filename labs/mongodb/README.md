...
- Наследование: Данная работа является продолжением всех предшествующих работ курса. Все механизмы аутентификации (JWT, OAuth), кеширования (Redis), документирования (OpenAPI) и бизнес-логика (CRUD, Soft Delete) должны оставаться работоспособными.
...
services:
  ...
  redis:
    # Конфигурация Redis из предыдущей работы остается без изменений
    ...
...
- Soft Delete: Реализуйте механизм мягкого удаления аналогично работе [Проектирование и реализация RESTful API](../rest-api) (например, поле `deletedAt` или `isDeleted`). Убедитесь, что запросы на получение данных автоматически фильтруют удаленные документы.
