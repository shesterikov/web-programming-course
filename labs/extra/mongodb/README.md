...
> Работа является дополнительной. Она не входит в обязательную последовательность работ курса и может быть выполнена после работы [Кеширование данных и управление сессиями с использованием Redis](../../redis-cache).
...
- Статус работы: Работа является дополнительной и не входит в обязательную часть курса. Она выполняется в отдельной ветке репозитория, чтобы основная ветка сохраняла реализацию на PostgreSQL, необходимую для последующих работ.
- Наследование: Данная работа является продолжением обязательных работ курса, предшествующих ей. Все механизмы аутентификации (JWT, OAuth), кеширования (Redis), документирования (OpenAPI) и бизнес-логика (CRUD, Soft Delete) должны оставаться работоспособными.
