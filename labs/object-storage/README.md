...
- Обязательно хранение признака видимости файла (приватный / публичный). Приватный файл доступен только владельцу, публичный — любому авторизованному пользователю.
...
- Наследование: Данная работа является продолжением всех предшествующих работ курса. Все механизмы аутентификации, кеширования и документирования должны оставаться работоспособными.
...
Существует два способа отдать файл клиенту: проксирование через API (приложение читает объект из хранилища и передает его в ответе) и Presigned URL (приложение выдает клиенту временную подписанную ссылку непосредственно на хранилище). В рамках данной работы используется проксирование через API — оно позволяет применить проверку прав доступа. Учтите, что при переходе на Presigned URL адрес подписывается тем значением, которое указано в `MINIO_ENDPOINT`: адрес `minio:9000` доступен только внутри сети Docker и не будет разрешен браузером, поэтому для внешних ссылок потребуется отдельный публичный адрес хранилища.
...
- Файл, назначенный аватаром, переводится в публичную видимость. Аватар отображается в интерфейсе рядом с именем пользователя и должен быть доступен не только владельцу, иначе профиль невозможно показать другим пользователям.
...
Создайте DTO для загрузки файла с полем файла типа multipart/form-data и необязательным полем видимости (по умолчанию — приватный). Создайте DTO для обновления профиля с опциональным полем avatarFileId типа UUID и другими полями профиля (displayName, bio).
...
| GET | /files/:fileId | Получение файла по ID | 200 OK | Владелец, либо любой авторизованный пользователь для публичного файла |
...
Контроллер файлов должен использовать декораторы для документации API, указывать возможные статусы ответов и потреблять multipart/form-data для загрузки. При отдаче файла необходимо устанавливать заголовки Content-Type, Content-Disposition и Content-Length.

Заголовок `Content-Disposition` управляет тем, как браузер поступит с ответом:
-   `inline` — содержимое отображается в самой странице. Этот режим необходим, чтобы изображение можно было вывести тегом `<img>`.
-   `attachment; filename="..."` — браузер предлагает сохранить файл на диск.

Реализуйте выбор режима через необязательный query-параметр (например, `?download=true`), используя `inline` в качестве значения по умолчанию для изображений. Оригинальное имя файла в заголовке должно быть корректно закодировано, если содержит символы кириллицы.
